...
public class WorkflowSocialSecurity extends BaseBean implements Action{
    private String interfaceName="社保申请审批对接HR接口";
    /**
     * @Author 李志辉
     * @Description 社保申请审批流程
     * @Date 2019/8/14
     * @Param [request]
    ...
    private JSONObject getParam(Map<String,String> mainTableDataMap, int requestid, String formTableName){
        ...
        json.put("ProcName", "WSPROC_Social_security");
        //考核月份
...
//        }
        ...
        sql="select bkrq,bksj,bkyy,bklx from "+formTableName+"_dt1 where mainid="+mainid;
        rs.execute(sql);
        while (rs.next()){
            ...
            tbody.put("ApplyDate",Util.null2String(rs.getString("bkrq")));
            tbody.put("EffectiveDate",Util.null2String(rs.getString("bksj")));
            tbody.put("YangLao",rs.getInt("bkyy"));
            tbody.put("YiLiao",Util.null2String(rs.getString("bklx")));
            tbody.put("ShengYu",Util.null2String(sprq));
            tbody.put("GongShang",Util.null2String(sprq));
            tbody.put("ShiYe",Util.null2String(sprq));
            list.add(tbody);
        }
