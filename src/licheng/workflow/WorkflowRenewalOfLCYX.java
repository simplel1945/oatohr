 ...
 *@创建时间  2020/5/9
 */
public class WorkflowRenewalOfLCYX extends BaseBean implements Action{
    private String interfaceName="续签劳动对接HR接口";
    /**
     * @Author 李志辉
     * @Description 续签劳动流程
     * @Date 2019/8/14
    ...
    private JSONObject getParam(Map<String,String> mainTableDataMap, int requestid, String formTableName){
        ...
        json.put("ProcName", "WSPROC_RenewalOfLC");
        //考核月份
...
//        }
        ...
        sql="select * from "+formTableName+"_dt1 where  mainid="+mainid;
        //sqrhr,xldhtqdrq,xldhtjsrq,oaid
        rs.execute(sql);
        while (rs.next()){
            ...
            //OA流程编号
            tbody.put("OAID",Util.null2String(Util.null2String(rs.getString("oaid"))));
            tbody.put("empcode",Util.null2String(rs.getString("sqrhr")));
            tbody.put("empname","");
            tbody.put("ApplyDate",Util.null2String(rs.getString("xldhtqdrq")));
            tbody.put("EffectiveDate",Util.null2String(rs.getString("xldhtjsrq")));
            tbody.put("EvaConclusion","建议续签");
            tbody.put("SIGNED",Util.null2String(mainTableDataMap.get("spjg")));
            list.add(tbody);
        }
