 ...
 *@创建时间  2020/5/9
 */
public class WorkflowEntryProcessJJ extends BaseBean implements Action{
    private String interfaceName="入职计件流程对接HR接口";
    /**
     * @Author 李志辉
     * @Description 入职计件流程流程
     * @Date 2019/8/14
     * @Param [request]
    ...
    private JSONObject getParam(Map<String,String> mainTableDataMap, int requestid, String formTableName){
        ...
        json.put("ProcName", "WSPROC_EntryProcess");
        ...
        //请假单ID
        sql="select rzrq,gh from "+formTableName+"_dt1 where mainid="+mainid;
        rs.execute(sql);
        while (rs.next()){
            ...
            tbody.put("EmployOAID","");
            tbody.put("empcode",Util.null2String(rs.getString("gh")));
            tbody.put("empname","");
            tbody.put("Email","");
            tbody.put("hiredate",Util.null2String(rs.getString("rzrq")));
            tbody.put("OpenEmailbox",Util.null2String(rs.getString("spjg")));
            tbody.put("SIGNED",Util.null2String(mainTableDataMap.get("spjg"))==""? 0:mainTableDataMap.get("spjg"));
            tbody.put("A01200",Util.null2String(mainTableDataMap.get("sjbgz"))==""? 0:mainTableDataMap.get("sjbgz"));
            tbody.put("A01201",Util.null2String(mainTableDataMap.get("syjxgz"))==""? 0:mainTableDataMap.get("syjxgz"));
            tbody.put("A01202",Util.null2String(mainTableDataMap.get("syjbgz"))==""? 0:mainTableDataMap.get("syjbgz"));
            tbody.put("A01XD","0");
            tbody.put("A01XJ","0");
            list.add(tbody);
        }
