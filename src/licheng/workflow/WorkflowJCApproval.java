 ...
 *@创建时间  2020/5/9
 */
public class WorkflowJCApproval extends BaseBean implements Action{
    private String interfaceName="奖惩审批对接HR接口";
    /**
     * @Author 李志辉
     * @Description 奖惩审批流程
     * @Date 2019/8/14
    ...
    public String execute(RequestInfo request){
        ...
        if("submit".equals(src)) {
            try {
                String accessToken = "";
                net.sf.json.JSONObject parameter = new net.sf.json.JSONObject();
                GeneralToolForHR gtfr = new GeneralToolForHR();
                net.sf.json.JSONObject Encrypt = gtfr.doPost(interfaceName, urlEncrypt, parameter, accessToken);
                writeLog(Encrypt);
    ...
    private net.sf.json.JSONObject getParam(Map<String,String> mainTableDataMap, int requestid, String formTableName){
        ...
        List<JSONObject> list=new ArrayList<net.sf.json.JSONObject>();
        RecordSet rs=new RecordSet();
        json.put("ProcName", "WSPROC_JCApproval");
        //考核月份
...
//        }
        ...
        while (rs.next()){
            ...
            tbody.put("Applicant","庄兴");
            tbody.put("ApplyDate","2019-06-11 00:00:00.000");
            tbody.put("EffectiveDate","2019-06-12 00:00:00.000");
            tbody.put("empcode","08000570");
            tbody.put("empname","");
            tbody.put("JCCategory","奖励");
            tbody.put("JCType","通报表扬");
            tbody.put("JCCause","加班改BUG");
            tbody.put("JCProposal","再接再厉");
            list.add(tbody);
        }
