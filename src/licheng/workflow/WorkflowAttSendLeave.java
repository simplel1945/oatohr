 ...
 *@创建时间  2020/5/9
 */
public class WorkflowAttSendLeave extends BaseBean implements Action{
    private String interfaceName="请假流程对接HR接口";
    @Override
    ...
    private JSONObject getParam(Map<String,String> mainTableDataMap, int requestid,String formTableName){
        ...
        json.put("ProcName", "WSPROC_AttSendLeave");
        //考核月份
...
//        }
        ...
        while (rs.next()){
            ...
            json_.put("OAID",Util.null2String(requestid));
            json_.put("OASubID",Util.null2String(requestid));
            json_.put("empcode",Util.null2String(mainTableDataMap.get("sqrhr")));
            json_.put("empname","");
            json_.put("LeaveClientele",Util.null2String(mainTableDataMap.get("dlr")));
            json_.put("LeaveType",Util.null2String(mainTableDataMap.get("qjlxhr")));
            json_.put("LeaveBegin",Util.null2String(mainTableDataMap.get("kssj")));
            json_.put("BeginTime",Util.null2String(mainTableDataMap.get("kssj1")));
            json_.put("LeaveEnd",Util.null2String(mainTableDataMap.get("jssj")));
            json_.put("EndTime",Util.null2String(mainTableDataMap.get("jssj1")));
            json_.put("LEAVE_BegNo","上半段");
            json_.put("LEAVE_EndNo","下半段");
            json_.put("LeaveDay",Util.null2String(mainTableDataMap.get("zts")));
            //json_.put("JB_DAY",Util.null2String(mainTableDataMap.get("jbrq")).length()==0? "1900-01-01":mainTableDataMap.get("jbrq"));
            //json_.put("JB_TIME",Util.null2String(mainTableDataMap.get("jbsjhr")));
            json_.put("LeaveHour",0);
            json_.put("LeaveStatus",1);
            json_.put("LeaveReason",Util.null2String(mainTableDataMap.get("ccsy")));
            json_.put("JB_DAY","1900-01-01");
            json_.put("JBKS_TIME","1900-01-01");
            json_.put("JBJS_TIME","1900-01-01");
            list.add(json_);
        }
        json.put("ProcParams", list);
        //writeLog(Util.null2String(mainTableDataMap.get("jbrq")).length());
